...
Database
└── Postgres
    ├── play_by_play                # ~2.69M rows
    └── shots (materialized view)   # field goal attempts only
...
### `play_by_play`
~7.53 raw play-by-play events. ~2.68M field goal attempts plus all other event types.
