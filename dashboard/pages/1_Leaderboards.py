...
st.caption(
    ...
    "xRAPM = same using expected shot value — more stable, less variance. "
    "Traded players shown as 2TM/3TM with aggregated stats."
)

SORT_OPTIONS = {
    "RAPM — Actual Impact":          "rapm",
    "xRAPM — Expected Impact":       "xrapm",
    "RAPM − xRAPM — Process Gap":    "rapm_vs_xrapm",
    "FG% vs Expected — Shot-Making": "fg_pct_above_expected",
    "Points Above Expected":         "shot_pts_above_expected",
    "Shot Quality (Avg xShot)":      "mean_xshot",
    "Points Per Game":               "ppg",
}
...
with tab_single:
    with st.expander("ℹ️ How to read this leaderboard", expanded=False):
        st.markdown(
            "**RAPM / xRAPM units:** points per 100 possessions above a league-average player. "
            "+2.0 = 2 extra points per 100 possessions. "
            "Requires ≥ the selected possession threshold for a stable estimate. "
    ...
    min_poss    = f4.number_input("Min Poss", min_value=100, max_value=5000,
                                  value=500, step=100, key="ss_minposs")
    top_n       = f5.number_input("Top N", min_value=5, max_value=50,
    ...
    if df.empty:
        ...
        val_labels = chart_df[sort_col].apply(
            lambda v: f"{v:+.3f}" if sort_col in ("rapm", "xrapm", "rapm_vs_xrapm",
                                                    "fg_pct_above_expected") else f"{v:.1f}"
        ).tolist()
        ...
        fig.add_vline(x=0, line_dash="dot", line_color="rgba(200,200,200,0.5)")
        fig.update_layout(
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "mean_xshot": st.column_config.NumberColumn(
                                  ...
                                  help="Average difficulty of shots attempted"),
...
with tab_pooled:
    with st.expander("ℹ️ About the v2 pooled model", expanded=False):
        st.markdown(
            "**Why pool 3 seasons?** More data per player reduces lineup collinearity. "
            "**Box-score prior?** Shrinks estimates toward each player's historical plus/minus "
            "baseline — correctly elevates stars. **RAPM+Prior is recommended** for comparing "
            "players across eras. See the Glossary for the full methodology."
        )
    ...
    if df_v2.empty:
        ...
        fig2.add_vline(x=0, line_dash="dot", line_color="rgba(200,200,200,0.5)")
        fig2.update_layout(
            title=f"Top {int(top_n_v2)} — RAPM + Prior  ({selected_window}  {pooled_stype})",
            yaxis=dict(autorange="reversed", automargin=True,
                       tickfont=dict(size=12)),
            xaxis_title="RAPM + Box-Score Prior (pts / 100 poss)",
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "rapm_prior":    st.column_config.NumberColumn(
                                     ...
                                     help="Recommended metric — pooled + shrunk toward plus/minus baseline"),
                "xrapm":         st.column_config.NumberColumn(
                                     "xRAPM", format="%.2f", width="small"),
                "rapm":          st.column_config.NumberColumn(
                                     "RAPM (raw)", format="%.2f", width="small"),
                "rapm_vs_xrapm": st.column_config.NumberColumn(
                                     "RAPM−xRAPM", format="%.2f", width="small"),
                "possessions":   st.column_config.NumberColumn(
                                     "Pooled Poss", format="%.0f", width="small"),
            },
        )
